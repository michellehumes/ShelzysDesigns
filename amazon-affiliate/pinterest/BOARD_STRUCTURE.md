# Pinterest Board Structure & Setup Guide

## Account Setup Checklist

### Step 1: Convert to Business Account
1. Go to [Pinterest Business](https://business.pinterest.com)
2. Click "Join as a business" or convert existing account
3. Fill in business details:
   - Business name: Shelzy's Designs
   - Website: [Your blog URL]
   - Business type: Blogger/Creator

### Step 2: Complete Profile
- **Profile Photo:** Logo or branded headshot
- **Display Name:** Shelzy's Designs | [Niche] (e.g., "Shelzy's Designs | Wedding & Lifestyle")
- **Bio:** (160 characters max)
  ```
  Curating the best Amazon finds for brides, preppy style lovers & organized homes. Personalized gifts, wedding essentials & aesthetic must-haves.
  ```
- **Website:** Your blog URL (claim your website for rich pins)

### Step 3: Enable Rich Pins
1. Go to Pinterest Settings > Claimed Accounts
2. Add your website URL
3. Verify using HTML tag, file upload, or DNS record
4. Apply for Rich Pins at [Pinterest Rich Pin Validator](https://developers.pinterest.com/tools/url-debugger/)

---

## Board Architecture

### Your Live Boards (@michelleshealthyhabit)

Your Pinterest boards are set up and ready! Here's how they map to product categories:

### Tier 1: Primary Boards (Wedding & Bridal Focus)

| Your Board Name | Product Categories to Pin | Priority |
|-----------------|---------------------------|----------|
| **Bridal Party Gifts** | Bridesmaid Proposal Gifts, Personalized Gifts | HIGH |
| **Bridesmaid Gifts & Ideas** | Bridesmaid Proposal Gifts, Wedding Day Essentials | HIGH |
| **Amazon Wedding Finds** | Wedding Day Essentials, Bridesmaid Proposal Gifts | HIGH |
| **Personalized Wedding...** | Personalized Gifts, Wedding Day Essentials | HIGH |
| **Bridal Shower Ideas** | Bridesmaid Proposal Gifts, Best Gifts Under $50 | HIGH |
| **Wedding Planning Tips...** | Wedding Day Essentials, All bridal content | HIGH |
| **Palm Beach Preppy Style** | Palm Beach Aesthetic Home Decor, Preppy Water Bottles | HIGH |

### Tier 2: Lifestyle & Home Boards

| Your Board Name | Product Categories to Pin | Priority |
|-----------------|---------------------------|----------|
| **Coastal Grandma...** | Palm Beach Aesthetic Home Decor | MEDIUM |
| **Home Decor &...** | Palm Beach Aesthetic Home Decor, Amazon Viral Organizers | MEDIUM |
| **Bathroom Organization** | Amazon Viral Organizers | MEDIUM |
| **Work from Home Office...** | Home Office Favorites, Creator Tools | MEDIUM |

### Tier 3: Beauty, Wellness & Self-Care Boards

| Your Board Name | Product Categories to Pin | Priority |
|-----------------|---------------------------|----------|
| **Amazon Beauty Favorites** | Amazon Beauty Must-Haves | MEDIUM |
| **Self-Care & Wellness** | Amazon Beauty Must-Haves, Fitness + Wellness Picks | MEDIUM |
| **Self Care** | Amazon Beauty Must-Haves, Fitness + Wellness Picks | MEDIUM |
| **Fitness & Wellness** | Fitness + Wellness Picks | MEDIUM |
| **Workouts** | Fitness + Wellness Picks | MEDIUM |
| **Stress Relief** | Fitness + Wellness Picks, Self-Care products | MEDIUM |
| **Weight Loss** | Fitness + Wellness Picks | MEDIUM |

### Tier 4: Travel, Baby & Pet Boards

| Your Board Name | Product Categories to Pin | Priority |
|-----------------|---------------------------|----------|
| **Packing & Travel...** | Travel Essentials | MEDIUM |
| **Travel & Honeymoon...** | Travel Essentials (honeymoon angle for brides!) | MEDIUM |
| **Nursery & Baby Prep** | Baby Prep Must-Haves | MEDIUM |
| **Dog Mom Essentials** | Dog Essentials | MEDIUM |

### Tier 5: Gift Guides

| Your Board Name | Product Categories to Pin | Priority |
|-----------------|---------------------------|----------|
| **Gift Guides (Under $50)** | Best Gifts Under $50, ALL categories | HIGH |

---

## Board-to-Category Mapping (Quick Reference)

Use this when deciding where to pin each product:

| Product Category | Pin To These Boards |
|------------------|---------------------|
| Bridesmaid Proposal Gifts | Bridal Party Gifts, Bridesmaid Gifts & Ideas, Amazon Wedding Finds, Bridal Shower Ideas |
| Wedding Day Essentials | Amazon Wedding Finds, Wedding Planning Tips..., Personalized Wedding..., Bridal Party Gifts |
| Personalized Gifts | Personalized Wedding..., Bridal Party Gifts, Gift Guides (Under $50) |
| Palm Beach Aesthetic Home Decor | Palm Beach Preppy Style, Coastal Grandma..., Home Decor &... |
| Preppy Water Bottles | Palm Beach Preppy Style, Gift Guides (Under $50) |
| Amazon Viral Organizers | Bathroom Organization, Home Decor &... |
| Amazon Beauty Must-Haves | Amazon Beauty Favorites, Self-Care & Wellness, Self Care |
| Travel Essentials | Packing & Travel..., Travel & Honeymoon... |
| Baby Prep Must-Haves | Nursery & Baby Prep |
| Kitchen Upgrades Under $30 | Home Decor &..., Gift Guides (Under $50) |
| Home Office Favorites | Work from Home Office... |
| Creator Tools | Work from Home Office... |
| Fitness + Wellness Picks | Fitness & Wellness, Workouts, Self Care, Stress Relief, Weight Loss |
| Dog Essentials | Dog Mom Essentials |
| Best Gifts Under $50 | Gift Guides (Under $50), ALL relevant boards |

---

## Blog Post to Board Mapping

Pin your existing blog posts to these boards:

| Blog Post | Pin To These Boards |
|-----------|---------------------|
| **Luxurious Bridesmaid Robes** | Bridal Party Gifts, Bridesmaid Gifts & Ideas, Amazon Wedding Finds |
| **Travel Jewelry Organizers for Weddings** | Packing & Travel..., Travel & Honeymoon..., Amazon Wedding Finds |
| **Bridal Party Gifts Under $50** | Gift Guides (Under $50), Bridal Party Gifts, Bridesmaid Gifts & Ideas |
| **Bachelorette Weekend Essentials** | Bridal Shower Ideas, Amazon Wedding Finds, Self Care |
| **How to Personalize Gifts Like a Pro** | Personalized Wedding..., Bridal Party Gifts, Wedding Planning Tips... |
| **What to Put in a Bridesmaid Proposal Box** | Bridesmaid Gifts & Ideas, Bridal Party Gifts, Wedding Planning Tips... |
| **Water Bottle Posts (6 total)** | Palm Beach Preppy Style, Gift Guides (Under $50) |

---

## Suggested Additional Boards (Optional)

Consider these as your content grows:

| Board Name | Purpose |
|------------|---------|
| **Holiday Gift Guide** | Seasonal (October-December) |
| **Kitchen Organization** | If you expand kitchen content |
| **Shelzy's Designs Shop** | Link directly to your Shopify products |

---

## Board Optimization Checklist

### For Each Board:
- [ ] Keyword-rich name (50 characters max)
- [ ] Compelling description (2-3 sentences, 500 characters max)
- [ ] Include 3-5 keywords naturally
- [ ] Add relevant hashtags (optional, 2-3 max)
- [ ] Set attractive board cover
- [ ] Categorize under appropriate Pinterest category

### Board Description Template
```
[What the board contains] for [target audience]. Discover [specific items 1], [specific items 2], and [specific items 3]. [Benefit statement]. #keyword1 #keyword2
```

**Example:**
```
The best bridesmaid proposal gifts and ideas for brides planning their wedding party. Discover personalized robes, will you be my bridesmaid boxes, and custom jewelry. Find the perfect way to ask your besties. #bridesmaidgifts #weddingplanning
```

---

## Board Cover Strategy

### Creating Consistent Covers
1. Use Canva (free) to create board covers
2. Size: 600 x 600 px (square format)
3. Include:
   - Board name in elegant typography
   - On-brand colors (blush, sage, gold)
   - Simple background or subtle pattern
   - Optional: small icon or graphic element

### Brand Color Palette for Covers
| Color Name | Hex Code | Use For |
|------------|----------|---------|
| Soft Black | #2C2C2C | Text |
| Brushed Beige | #F5F1EB | Background |
| Sage Green | #A8B5A0 | Accents (lifestyle) |
| Blush Apricot | #E8C4B8 | Accents (bridal) |
| Dusty Seagrass | #B8C5B9 | Accents (home) |

---

## Pin Distribution Strategy

### Pins Per Board (Targets)

| Board Tier | Initial Pins | Monthly Adds | Total Goal (Year 1) |
|------------|--------------|--------------|---------------------|
| Primary | 50 | 30 | 400+ |
| Secondary | 30 | 20 | 250+ |
| Tertiary | 20 | 15 | 200+ |
| Seasonal | 20 | 10 (when active) | 100+ |
| Lifestyle | 30 | 10 | 150+ |

### Content Mix Per Board
- **70%** Your original content (blog pins, product pins)
- **30%** Quality repins from others (engagement, variety)

---

## Board Maintenance

### Weekly Tasks
- [ ] Add 3-5 new pins to each primary board
- [ ] Repin 5-10 quality pins from others
- [ ] Check board covers display correctly
- [ ] Respond to any board comments

### Monthly Tasks
- [ ] Review board analytics (top performing boards)
- [ ] Update board descriptions if needed
- [ ] Archive underperforming boards
- [ ] Create seasonal boards as needed

### Quarterly Tasks
- [ ] Full board audit
- [ ] Update all board covers for consistency
- [ ] Remove broken or irrelevant pins
- [ ] Reorganize board order by priority

---

## Group Boards Strategy (Optional)

### Finding Group Boards
1. Search Pinterest for "[your niche] group board"
2. Use [PinGroupie](https://pingroupie.com) to find active boards
3. Look at competitors' group board memberships

### Joining Group Boards
- Read board rules carefully
- Follow the board owner
- Send a join request via the method specified (usually email or Pinterest message)
- Use the email template in `/resources/GROUP_BOARD_REQUEST.md`

### Group Board Best Practices
- Follow all board rules exactly
- Pin consistently but don't spam
- Engage with other pinners' content
- Share only your highest-quality pins

---

## Board Order (Profile Display)

Arrange your boards on your profile strategically:

**Top Row (Highest Priority - Wedding/Bridal)**
1. Bridal Party Gifts
2. Amazon Wedding Finds
3. Personalized Wedding...
4. Bridal Shower Ideas
5. Gift Guides (Under $50)
6. Palm Beach Preppy Style

**Second Row (Lifestyle & Home)**
7. Coastal Grandma...
8. Home Decor &...
9. Amazon Beauty Favorites
10. Self-Care & Wellness
11. Packing & Travel...
12. Travel & Honeymoon...

**Third Row (Niche Categories)**
13. Bathroom Organization
14. Work from Home Office...
15. Fitness & Wellness
16. Nursery & Baby Prep
17. Dog Mom Essentials

**To Reorder on Pinterest:**
1. Go to your profile
2. Click "Organize" or drag boards
3. Place highest-converting boards first

---

## Analytics to Track

### Board-Level Metrics
- **Impressions:** How often pins are seen
- **Engagements:** Saves + clicks
- **Top Pins:** Which pins perform best
- **Audience:** Who's engaging

### Review Schedule
- **Weekly:** Check top performing boards
- **Monthly:** Review all board analytics
- **Quarterly:** Adjust strategy based on data

### What to Do with Data
- **High impressions, low clicks:** Improve pin design/CTAs
- **Low impressions:** Add more pins, optimize descriptions
- **High engagement board:** Create more content for that topic
- **Low engagement board:** Consider archiving or refreshing
