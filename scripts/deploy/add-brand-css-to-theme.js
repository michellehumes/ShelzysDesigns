...
async function main() {
  ...
  // Step 2: Add CSS file as asset (use v2 filename to avoid generated asset conflict)
  console.log('📄 Adding shelzys-brand-v2.css to assets...');
  ...
  const cssAssetResponse = await apiRequest('PUT', `/themes/${liveTheme.id}/assets.json`, {
    asset: {
      key: 'assets/shelzys-brand-v2.css',
      value: cssContent
    }
  });

  if (cssAssetResponse.status === 200 || cssAssetResponse.status === 201) {
    console.log('   ✅ shelzys-brand-v2.css added successfully!');
  } else {
